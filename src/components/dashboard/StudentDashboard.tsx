...
const StudentDashboard: React.FC = () => {
    ...
    const fetchSchedule = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const processed = data.map((cls: any) => ({
            ...cls,
  ...
  const gradeColumns = [
    ...
    { key: "assignment", label: "Assignment", sortable: true },
    {
      ...
      render: (grade: string) => (
        <span
          className={`px-3 py-1 rounded-xl text-xs font-black tracking-tight ${parseFloat(grade) >= 90
    ...
    const fetchAnnouncements = async () => {
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          // Filter by enrolment
          const filtered = data.filter((ann: any) => enrolledClassIds.includes(ann.classId));
            ...
            <div className="bg-white rounded-[2rem] shadow-sm border border-gray-100 p-8 relative overflow-hidden h-full">
              ...
              <div className="space-y-5">
                {announcements.length > 0 ? announcements.map((ann, idx) => {
                  const sourceClass = todayClasses.find(c => c.classId === ann.classId);
                  return (
