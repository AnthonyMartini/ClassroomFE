...
import { UserPlus, Shield, CheckCircle, Search, Mail, X, Edit2, Check } from "lucide-react";
...
const ParentAccess: React.FC = () => {
    ...
    const fetchLinkedParents = async () => {
        if (!user) return;
        try {
            const response = await fetch(`http://localhost:5000/api/parent-links?studentId=${user.id}`);
            if (response.ok) {
    ...
    const handleGrantAccess = async () => {
        ...
        try {
            ...
            if (response.ok) {
                setMessage({ type: 'success', text: "Access granted! Your parent can now see your grades and attendance." });
                setEmail("");
                fetchLinkedParents();
            } else {
            ...
            <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-xl overflow-hidden relative">
                ...
                <div className="relative z-10">
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <div>
                            <h1 className="text-3xl font-black text-gray-900 tracking-tight">Parent Access</h1>
                            <p className="text-gray-500 font-medium tracking-tight">Manage who can view your academic progress</p>
                        </div>
                    </div>
                    ...
                    <div className="bg-blue-50/50 border border-blue-100 p-6 rounded-2xl mb-8">
                        ...
                        <p className="text-sm text-blue-800 leading-relaxed">
                            To protect your privacy, parents do not have access to your grades by default.
                            When you add a parent's email below, they will be able to see your overall
                            grades in each course and your attendance stats on their dashboard.
                        </p>
                    </div>
                    ...
                    <div className="space-y-6">
                        <div>
                            ...
                            <div className="flex gap-3">
                                ...
                                >
                                    {isSubmitting ? "Processing..." : <><UserPlus size={18} /> Grant Access</>}
                                </button>
                            </div>
                        </div>

                        {message && (
                            <div className={`p-4 rounded-2xl flex items-center gap-3 animate-in zoom-in-95 duration-300 ${message.type === 'success' ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-red-50 text-red-700 border border-red-100'
                                }`}>
                ...
                {linkedParents.length > 0 ? (
                    <div className="space-y-4">
                        {linkedParents.map((parent, idx) => (
                            <div key={idx} className="flex items-center justify-between p-4 bg-gray-50/50 rounded-2xl border border-gray-100 group">
                                    ...
                                    <div className="flex flex-col">
                                        {editingEmail === parent.parentEmail ? (
                                            <div className="flex gap-2">
                                                ...
                                                />
                                                <button
                                                    onClick={() => handleUpdateNickname(parent.parentEmail)}
                                            ...
                                            <div className="flex items-center gap-3">
                                                <span className="font-bold text-gray-900">{parent.nickname}</span>
                                                {parent.nickname !== parent.parentEmail && (
                                                    <span className="text-xs text-gray-400 font-medium">({parent.parentEmail})</span>
                                                )}
                                                <button
                                                    onClick={() => {
                                                        setEditingEmail(parent.parentEmail);
                                                        setNewNickname(parent.nickname || parent.parentEmail);
                                                    }}
                    ...
                    <div className="text-center py-12 bg-gray-50/50 rounded-2xl border border-dashed border-gray-200">
                        ...
                        <p className="text-gray-500 font-medium">None of your parents have access yet.</p>
                    </div>
                )}
            </div>
        </div>
